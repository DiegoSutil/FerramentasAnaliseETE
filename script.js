...
// Funções utilitárias e de cálculo (definidas no escopo global para serem acessíveis)
...
window.displayHistory = function(displayElementId, data, loadFunction, collectionName, appId) {
    ...
    displayElement.querySelectorAll('.delete-btn').forEach(button => {
        button.addEventListener('click', async (e) => {
            ...
            try {
                if (db && currentUserId && appId) { // Usa as variáveis do âmbito do módulo
                   await deleteDoc(doc(db, `artifacts/${appId}/users/${currentUserId}/${collectionName}`, id)); // Usa as variáveis do âmbito do módulo
...
const addEventListeners = () => {
    document.getElementById('showSludgeAge')?.addEventListener('click', () => { console.log('Show Sludge Age clicked'); window.showSection('sludgeAgeSection'); });
    document.getElementById('showPhysicalChemical')?.addEventListener('click', () => { console.log('Show Physical Chemical clicked'); window.showSection('physicalChemicalSection'); });
    document.getElementById('showOrganicLoad')?.addEventListener('click', () => { console.log('Show Organic Load clicked'); window.showSection('organicLoadSection'); });
    document.getElementById('showHowItWorks')?.addEventListener('click', () => { console.log('Show How It Works clicked'); window.showSection('howItWorksSection'); });

    document.getElementById('calculateButton')?.addEventListener('click', () => { console.log('Calculate Sludge Age clicked'); window.calculateSludgeAge(); });
    document.getElementById('calculatePhysicalChemical')?.addEventListener('click', () => { console.log('Calculate Physical Chemical clicked'); window.calculatePhysicalChemicalEfficiency(); });
    document.getElementById('calculateOrganicLoadButton')?.addEventListener('click', () => { console.log('Calculate Organic Load clicked'); window.calculateOrganicLoad(); });

    document.getElementById('saveSludgeAgeData')?.addEventListener('click', async () => {
        console.log('Save Sludge Age clicked');
        const data = await window.calculateSludgeAge();
        if (data) {
            // Acessa firebaseConfig para obter o projectId
            const currentAppIdValue = firebaseConfig && firebaseConfig.projectId ? firebaseConfig.projectId : 'default-app-id';
    ...
    document.getElementById('savePhysicalChemicalData')?.addEventListener('click', async () => {
        console.log('Save Physical Chemical clicked');
    ...
    document.getElementById('saveOrganicLoadData')?.addEventListener('click', async () => {
        console.log('Save Organic Load clicked');
    ...
    if (analyzeCsvButton) {
        analyzeCsvButton.addEventListener('click', () => {
            console.log('Analyze CSV clicked');
