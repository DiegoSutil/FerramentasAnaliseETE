...
// Configuração do Firebase - Usará a configuração real se injetada, senão será nula
// Note: Esta configuração DEVE vir do seu projeto Firebase.
// No GitHub Pages, as variáveis __firebase_config etc. NÃO são injetadas.
...
document.addEventListener('DOMContentLoaded', async () => {
    console.log("DOM totalmente carregado. Tentando inicializar Firebase...");
    
    // Inicializar Firebase se a configuração estiver disponível
    if (firebaseConfig && firebaseConfig.apiKey) {
        try {
            console.log("Configuração do Firebase encontrada. Prosseguindo com a inicialização.");
            ...
            onAuthStateChanged(auth, async (user) => {
                if (user) {
                    ...
                    try {
                        ...
                        console.log("Autenticação concluída. Usuário:", currentUserId);
                    } catch (error) {
                        console.error("Erro na autenticação:", error);
                        currentUserId = crypto.randomUUID(); // Fallback para manter a UI funcional
        ...
        console.warn("Configuração do Firebase (firebaseConfig) não encontrada. As funcionalidades de base de dados (salvar/carregar histórico) NÃO ESTÃO ATIVAS neste ambiente.");
    ...
    // --- Funções de Ajuda, Cálculo e Salvar (definidas no âmbito global para acessibilidade) ---
    ...
    window.saveData = async function(collectionName, data, appId) {
        ...
        try {
            ...
            alert("Dados salvos com sucesso!"); // Feedback de sucesso
            return docRef.id;
        } catch (e) {
            console.error("Erro ao adicionar documento: ", e);
            alert(`Erro ao salvar os dados: ${e.message}. Verifique as regras de segurança do Firestore.`);
        }
    };

    window.setupHistoryListener = function(collectionName, historyDisplayId, loadFunction, appId) {
        console.log(`Configurando listener para histórico da coleção: ${collectionName}`);
        if (!isFirebaseInitialized || !db || !currentUserId || !appId) {
            console.warn(`AVISO: Firestore não inicializado ou ID do usuário/appId não disponível. Não foi possível carregar o histórico para ${collectionName}.`);
        ...
        onSnapshot(q, (snapshot) => {
            console.log(`Dados recebidos para o histórico da coleção: ${collectionName}`);
            const historyData = [];
    ...
    window.displayHistory = function(displayElementId, data, loadFunction, collectionName, appId) {
        console.log(`Exibindo histórico para: ${displayElementId}`);
        const displayElement = document.getElementById(displayElementId);
        if (!displayElement) {
            console.warn(`Elemento de exibição de histórico ${displayElementId} não encontrado.`);
            return;
        }
        ...
        displayElement.querySelectorAll('.load-btn').forEach(button => {
            button.addEventListener('click', (e) => {
                ...
                if (entryToLoad) {
                    ...
                    console.log(`Carregado histórico para edição: ${id}`);
                }
            });
        });
        ...
        displayElement.querySelectorAll('.delete-btn').forEach(button => {
            button.addEventListener('click', async (e) => {
                ...
                // Substituindo o confirm() por feedback no console para ambientes sem pop-ups
                if (isFirebaseInitialized && db && currentUserId && appId) { // Verifica se Firebase está inicializado
                    try {
                        await deleteDoc(doc(db, `artifacts/${appId}/users/${currentUserId}/${collectionName}`, id));
                        console.log("Documento excluído com sucesso!");
                        alert("Registro excluído com sucesso!"); // Feedback de sucesso
                    } catch (e) {
                   ...
                   alert("Não foi possível excluir o registro. Funcionalidades de base de dados inativas.");
                }
            });
        });
    };
    ...
    window.showSection = function(sectionId) {
        ...
        console.log('Estilos dos botões de navegação atualizados.');
    };

    // Adicionar event listeners após o DOM ser carregado
    const addEventListeners = () => {
        console.log("Anexando event listeners aos botões...");
        document.getElementById('showSludgeAge')?.addEventListener('click', () => { console.log('showSludgeAge clicked'); window.showSection('sludgeAgeSection'); });
        document.getElementById('showPhysicalChemical')?.addEventListener('click', () => { console.log('showPhysicalChemical clicked'); window.showSection('physicalChemicalSection'); });
        document.getElementById('showOrganicLoad')?.addEventListener('click', () => { console.log('showOrganicLoad clicked'); window.showSection('organicLoadSection'); });
        document.getElementById('showHowItWorks')?.addEventListener('click', () => { console.log('showHowItWorks clicked'); window.showSection('howItWorksSection'); });

        document.getElementById('calculateButton')?.addEventListener('click', () => { console.log('calculateButton clicked'); window.calculateSludgeAge(); });
        ...
        if (analyzeCsvButton) {
            analyzeCsvButton.addEventListener('click', () => {
                console.log('analyzeCsvButton clicked');
        ...
        if (csvDropZone) {
            csvDropZone.addEventListener('dragover', (e) => {
                ...
                console.log('CSV dragover');
            });

            csvDropZone.addEventListener('dragleave', () => {
                csvDropZone.classList.remove('drag-over');
                console.log('CSV dragleave');
            });
        ...
        console.log("Todos os event listeners foram anexados.");
    };
